  ...
  return {
    ...
    transformBundle (code) {
      ...
      for (name in found) {
        ...
        const negs = negatives[name] = []
        // add a negative for each positive
        for (let i = values.length - 1, neg; i >= 0; i--) {
          if (negs.indexOf(neg = !values[i]) === -1) negs.push(neg)
        }
        // add the negatives to the values
        for (let i = negs.length - 1, neg; i >= 0; i--) {
          if (values.indexOf(neg = negs[i]) === -1) values.push(neg)
        }
      }
      ...
      for (name in found) {
        for (let i = 0, l = objs.length; i < l; i++) {
          ...
          if (val.constructor === Array) {
            for (let j = val.length - 1, cl; j >= 0; j--) {
              cl = Object.assign({}, obj)
              cl[name] = val[j]
              objs.push(cl)
            }
            objs.splice(i--, 1)
