...
module.exports = (code, imports, globals, storage) => falafel({
  ...
  const TEMP = placeholder('vigour-ua|navigator')
  ...
  if (type === 'ImportDeclaration' && imports) {
    ...
    if (resolved !== void 0) {
      // remove these hacks!
      // what we need to do is match imports during regular transform
      // replace them with placeholder vars
      // match the placeholder vars
      if (`"${resolved}"` === TEMP) {
        mydecl(node.parent.kind, node)[node.id.name] = 'vigour-ua|navigator'
      } else {
